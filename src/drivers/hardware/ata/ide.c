...
/*void detect_irq_need(){
    outportb((1 << 31) | (bus << 16) | (device << 11) | (func << 8) | 8, 0xCF8); // Send the parameters.
    if ((inl(0xCFC) >> 16) != 0xFFFF) { // If device exists (class isn't 0xFFFF)
                                        // Check if this device needs an IRQ assignment:
        outportb((1 << 31) | (bus << 16) | (device << 11) | (func << 8) | 0x3C, 0xCF8); // Read the interrupt line field
        outportb(0xFE, 0xCFC); // Change the IRQ field to 0xFE
        outportb((1 << 31) | (bus << 16) | (device << 11) | (func << 8) | 0x3C, 0xCF8); // Read the interrupt line field
...
unsigned char ide_print_error(unsigned int drive, unsigned char err) {
    const char *types_prim[] = {"Primary", "Secondary"};
    const char *types_drive[] = {"Master", "Slave"};
    ...
    puts("- [");
    puts(types_prim[ide_devices[drive].Channel]);
    puts(" - ");
    puts(types_drive[ide_devices[drive].Drive]); // Same as above, using the drive
    puts(ide_devices[drive].Model);
...
void ide_initialize(unsigned int BAR0, unsigned int BAR1, unsigned int BAR2, unsigned int BAR3,
    ...
    for (i = 0; i < 2; i++)
        for (j = 0; j < 2; j++) {
            ...
            timer_wait(1); // This function should be implemented in your OS. which waits for 1 ms.
    ...
    for (i = 0; i < 4; i++)
        if (ide_devices[i].Reserved == 1) {
            puts(" Found ");
            puts((const char *[]){"ATA", "ATAPI"}[ide_devices[i].Type]);/* Type */
            puts(" Drive ");
            puts_decimal(ide_devices[i].Size / 1024 / 1024 / 2);
            puts("GB - ");
